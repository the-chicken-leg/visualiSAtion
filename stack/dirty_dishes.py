...
class DirtyDishes:
    ...
    def draw_in_window(self, window: gr.Window, single_dish_height: int, sleep_time: float):
        self.window = window
        ...
        for i, dirty_dish in enumerate(self.dirty_dishes):
            ...
            dirty_dish.draw(window, gr.Point(self.center_x, center_y), single_dish_height)
        window.redraw()
    ...
    def push(self):
        ...
        new_dish.draw(self.window, gr.Point(self.center_x, center_y), self.single_dish_height)
        self.window.redraw()
